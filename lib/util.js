...
const checkAST = require('ast-validator')
...
util.enableTestMode = () => {
  ensure = function ensure(kind, value) {
...
util.declaration = (name, init) => ensure('statement', {
  ...
  kind: 'var',
  declarations: [
    { type: 'VariableDeclarator', id: util.identifierIfString(name), init: init && ensure('expression', init) }
  ]
})
