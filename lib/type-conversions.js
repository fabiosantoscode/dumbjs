    ...
    enter: function (node) {
      ...
      if (node.type === 'BinaryExpression') {
        ...
        if (
          ...
          return call('JS_ADD', [ node.left, node.right ])
        }
        if (node.operator === '+' &&
          ...
          if (leftType.name !== 'string') {
            ...
            node.left = call('String', [node.left])
          }
          if (rightType.name !== 'string') {
            //   b. Let rstr be ? ToString(rprim).
            node.right = call('String', [node.right])
          }
          ...
          return {
            ...
            left: leftType.name !== 'number' ? call('Number', [ node.left ]) : node.left,
            right: rightType.name !== 'number' ? call('Number', [ node.right ]) : node.right,
          }
        }
      }
      if (node.type === 'UnaryExpression') {
        if (node.operator === '-') {
          return {
            ...
            operator: node.operator,
            argument: call('Number', [node.argument]),
            prefix: node.prefix,
          }
        }
        if (node.operator === '+') {
          return call('Number', [node.argument])
        }
      }
    }
  })
